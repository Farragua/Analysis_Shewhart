function [r, num_aportaciones, valor_final_total, detalle] = Strategy_event_driven_rebal_mm_flexible_v1_sigma( ...
    periodos, basePath, tickers_config, weights_config, z_engine, n_boll, z_threshold, MA_sig, MA_reb, max_cash_por_dia, upper_abs, debug)

% Portfolio Event-Driven + Reb. SUAVE con MM de receptores flexible (Px<MMreb)
% CAMBIOS MINIMOS:
...
if nargin<10||isempty(max_cash_por_dia), max_cash_por_dia=1500; end
if nargin<11||isempty(upper_abs), upper_abs=0.30*ones(1,length(tickers_config)); end
if nargin<12||isempty(debug),   debug=false; end
...
for i=1:N
    ...
    % ===== (3) Rebalanceo suave con MM de receptores (Px<MMreb) =====
    ...
    if invertido>0
        ...
        if any(over)
            ...
            if any(rec)
                ...
                % 2) Repartir resto por pesos objetivo entre receptores
                if cash_tmp>1e-12
            ...
            % Actualizar unidades tras ventas/compras de rebalanceo
            acc=valores./precios;
        end
    end

    % ===== (4) Compras por senal (con tope diario) =====
    ...
    % (5) Tracking de equity (INCLUYE flujos del propio día)
...
detalle.params = struct('periodos', periodos, 'aportacion_mensual', aportacion_mensual, ...
                       ...
                       'max_cash_por_dia', max_cash_por_dia);

% ---------- Print ----------
if ~debug
    fprintf('=====================================================================================================================\n');
    fprintf('Portfolio Event-Driven + Reb. SUAVE (MMrec) (%s) | %d anos | CAGR: %.2f%% | Aportes: %d | Valor final: %.2f\n', ...
    ...
    fprintf('=====================================================================================================================\n');
    fprintf('Portfolio Event-Driven + Rebalanceo SUAVE con MM de receptores Flexible\n');
    fprintf('Tickers: %s\n', strjoin(tickers_config, ', '));
    fprintf('Pesos objetivo: %s\n', sprintf('%.1f%% ', weights_config*100));
    fprintf('Motor: %s%s | Tope diario: %.0f EUR\n', ...
        z_engine, iff(strcmpi(z_engine,'bollinger'), sprintf('(n=%d)',n_boll), ''), max_cash_por_dia);
