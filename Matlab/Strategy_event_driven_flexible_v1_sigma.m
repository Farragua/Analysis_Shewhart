function [r, num_aportaciones, valor_final_total, detalle] = Strategy_event_driven_flexible_v1_sigma( ...
    periodos, basePath, tickers_config, weights_config, z_engine, n_boll, z_threshold, MA, max_cash_por_dia, debug)
...
P_w      = table2array(TTw);  % Convertir a matriz numerica
N_w      = size(P_w,1);
...
for i=1:N
    ...
    if i == 1
        ...
        valor_inicio = sum(acc .* precios_prev) + cash_bal; % valor total al inicio
        if valor_inicio > 0
            delta_activos = sum(acc .* (precios_dia - precios_prev)); % cash sin rendimiento
    ...
    if cash_bal>1e-12 && any(sig(i,:))
        ...
        if presupuesto>0
            ...
            idx=find(sig(i,:)); 
            amount_per=presupuesto/numel(idx);
            for c=idx(:).'
                acc(c)=acc(c)+amount_per/precios_dia(c);
...
if ~debug
    fprintf('=====================================================================================================================\n');
    fprintf('Portfolio Event-Driven (%s) | %d anos | CAGR: %.2f%% | Aportes: %d | Valor final: %.2f\n', ...
    ...
    fprintf('=====================================================================================================================\n');
    fprintf('Portfolio Event-Driven Flexible (sin rebalanceo)\n');
        ...
        if exist(nm,'file')==2 || exist(nm,'class')==8
            ...
            if ~isempty(H)
                if isstruct(H) && isfield(H,'read_yahoo_csv') && isa(H.read_yahoo_csv,'function_handle')
                    reader=H.read_yahoo_csv; irrsolve=H.solve_irr; shew=H.shewhart_tasa_const; return;
                else
