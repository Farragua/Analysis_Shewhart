...
def Strategy_event_driven_rebal_mm_flexible_v1_sigma(
        ...
        max_cash_por_dia=None, upper_abs=None, debug=None):
    # ---------- Defaults ----------
    ...
    MMreb_w = np.zeros((N_w, num_activos))
    for j in range(1, num_activos + 1):
        MMsig_w[:, j - 1] = pd.Series(P_w[:, j - 1]).rolling(MA_sig, min_periods=1).mean().to_numpy()
        MMreb_w[:, j - 1] = pd.Series(P_w[:, j - 1]).rolling(MA_reb, min_periods=1).mean().to_numpy()
    ...
    for i in range(1, N + 1):
        ...
        if cash_bal > 1e-12 and np.any(sig[i - 1, :]):
            ...
            if presupuesto > 0:
                ...
                for c in idx:
                    acc[c - 1] = acc[c - 1] + amount_per / precios[c - 1]
    ...
    if not debug:
        print('=' * 117)
        print('Portfolio Event-Driven + Reb. SUAVE (MMrec) (%s) | %d anos | CAGR: %s%% | Aportes: %d | Valor final: %s'
        ...
        print('=' * 117)
        print('Portfolio Event-Driven + Rebalanceo SUAVE con MM de receptores Flexible')
        print('Tickers: %s' % ', '.join(tickers_config))
        print('Pesos objetivo: %s' % ''.join('%.1f%% ' % (x * 100) for x in np.asarray(weights_config).ravel()))
        print('Motor: %s%s | Tope diario: %s EUR'
              % (z_engine,
                 _iff(z_engine.lower() == 'bollinger', '(n=%d)' % int(n_boll), ''),
                 _fmt(max_cash_por_dia, '.0f')))
        print('Senal: z<=%s & Px<MMsig(%d) | Receptores: Px<MMreb(%d)'
              % (_fmt(z_threshold, '.2f'), int(MA_sig), int(MA_reb)))
