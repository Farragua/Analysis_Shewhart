...
def Strategy_event_driven_rebal_flexible_v1_sigma(
    ...
    for i in range(1, N + 1):
        ...
        if invertido > 0:
            ...
            if np.any(over):
                ...
                activaciones_count = activaciones_count + over.astype(np.float64)
                rec = ~over
                if np.any(rec):
                    deficit = np.maximum(target[rec] - valores[rec], 0)
                    sdef = np.sum(deficit)
                    if sdef > 0:
                        valores[rec] = valores[rec] + np.sum(exceso) * (deficit / sdef)
                acc = valores / precios
    ...
    if not debug:
        # Formato simple
        print('=' * 117)
        print('Portfolio Event-Driven + Reb. SUAVE (%s) | %d anos | CAGR: %s%% | Aportes: %d | Valor final: %s'
        ...
        print('=' * 117)
        print('Portfolio Event-Driven + Rebalanceo SUAVE Flexible (instantaneo)')
        ...
        print('-' * 117)
        print('METRICAS DE SENALES Y ACTIVACIONES:')
        ...
        print('  Activaciones de rebalanceo por activo: %s'
              % ''.join('%s=%d ' % (tickers_config[k], activaciones_count[k]) for k in range(num_activos)))
        print('-' * 117)
        print('DESGLOSE POR ACTIVO a %s:' % pd.Timestamp(dates[-1]).strftime('%Y-%m-%d'))
        for j in range(1, num_activos + 1):
