...
def Strategy_event_driven_rebal_cashpool_flexible_v2_sigma(
        periodos=None, basePath=None, tickers_config=None, weights_config=None,
        z_engine=None, n_boll=None, z_threshold=None, MA=None, upper_abs=None,
        debug=None):
    # ---------- Defaults ----------
    ...
    # Tracking
    equity_series = np.zeros(N)        # equity post-flujos (activos + cash)
    r_port = np.full(N, np.nan)        # retornos TWR diarios ANTES de flujos (cash = 0%)
    compras_idx = []
    compras_dst = []
    compras_amt = []
    compras_px = []
    ...
    if not debug:
        print('=' * 117)
        print('Portfolio Event-Driven + Reb. a CASH v2 (%s) | %d anos | CAGR: %s%% | Aportes: %d | Valor final: %s'
        ...
        print('=' * 117)
        print('Portfolio Event-Driven + Rebalanceo por bandas a CASH v2 - SIN LIMITES DE CAPITAL')
