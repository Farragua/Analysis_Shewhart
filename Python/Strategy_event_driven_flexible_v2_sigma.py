...
def Strategy_event_driven_flexible_v2_sigma(periodos=None, basePath=None, tickers_config=None,
                                            weights_config=None, z_engine=None, n_boll=None,
                                            z_threshold=None, MA=None, debug=None):
    # ---------- Defaults ----------
    ...
    for i in range(1, N + 1):
        ...
        if i == 1:
            ...
            if valor_inicio > 0:
                delta_activos = np.sum(acc * (precios_dia - precios_prev))
        ...
        if np.any(sig[i - 1, :]):
            ...
            for c in idx:
                acc[c - 1] = acc[c - 1] + amount_per_signal / precios_dia[c - 1]
    ...
    if not debug:
        print('=' * 117)
        print('Portfolio Event-Driven v2 (%s) | %d anos | CAGR: %s%% | Aportes: %d | Valor final: %s'
        ...
        print('=' * 117)
        print('Portfolio Event-Driven v2 - SIN LIMITES DE CAPITAL')
        print('Tickers: %s' % ', '.join(tickers_config))
        print('Pesos objetivo: %s' % ''.join('%.1f%% ' % (w * 100) for w in np.asarray(weights_config).ravel()))
        print('Motor: %s%s | Senal: z<=%s & Px<MM(%d) | Inversion por senal: %s EUR'
              % (z_engine,
                 _iff(z_engine.lower() == 'bollinger', '(n=%d)' % int(n_boll), ''),
                 _fmt(z_threshold, '.2f'), int(MA), _fmt(amount_per_signal, '.0f')))
        ...
        print('-' * 117)
        print('METRICAS DE APALANCAMIENTO:')
        if inversiones_por_senal.shape[0] > 0:
            print('  Dias con inversion > aporte mensual: %d' % inversiones_por_senal.shape[0])
            print('  Inversion maxima en un dia: %s EUR'
                  % _fmt(np.max(inversiones_por_senal[:, 1]), '.2f'))
        else:
            print('  Sin dias de inversion superior al aporte mensual')
        print('-' * 117)
