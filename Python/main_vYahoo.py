...
def main_vYahoo():
    ...
    assets = ["MSFT", "GOOG", "META", "AMZN", "MA", "V", "ASTS", "SLS", "UBER", "MCD", "AENA.MC", "GC=F"]
    ...
    for i in range(1, n_iteraciones + 1):
        ...
        msg = sendNotification(asset, data, entradas_2smm, entradas_2mmvix, mm200[-1])
        print(msg)
    ...
    # =========================================================================
    asunto = f'📊 Reporte Diario de Trading - {datetime.now().strftime("%Y-%m-%d")}'
    enviar_reporte(asunto, msg_final)

    # ------------------------------------------------------------------------------------------------------------------------
    # PORTFOLIO ANALYSIS: (comentado en el original MATLAB)
    #
    # Cartera All-Weather
